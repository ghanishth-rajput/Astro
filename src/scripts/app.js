...
const populateAadhaarFields = () => {
    return `
        <div class='formContainer' id='aadhar'>
            ...
            <label for="gender_aadhaar">Gender:</label>
            <select id="gender_aadhaar" name="gender" required>
...
const populateDrivingLicenseFields = () => {
    return `
        <div class='formContainer' id='driver'>
            ...
            <input type="date" id="DOE_drivingLicense" name="DOE" required>
        </div>
    `;
};
...
const populatePanCardFields = () => {
    return `
        <div class='formContainer' id='pan'>
            ...
            <label for="gender_panCard">Gender:</label>
            <select id="gender_panCard" name="gender" required>
                <option value="MALE">MALE</option>
                <option value="FEMALE">FEMALE</option>
            </select>
        </div>
...
documentTypeSelect.addEventListener("change", () => {
    ...
    documentFieldsDiv.style.display = selectedDocumentType ? "block" : "none";
});
...
const editItem = (event) => {
    const item = event.target.closest(".item");
    const documentType = item.querySelector("div:nth-child(1)").textContent.trim();
    ...
    const documentFieldsHTML = populateFields(mappedDocumentType);
    documentFieldsDiv.innerHTML = documentFieldsHTML;
    documentFieldsDiv.style.display = "block";

    documentForm.querySelector("#documentNumber_" + documentType.toLowerCase()).value = documentNumber;
    documentForm.querySelector("#holdingPersonName_" + documentType.toLowerCase()).value = holdingPersonName;
    documentForm.querySelector("#DOB_" + documentType.toLowerCase()).value = DOB;
